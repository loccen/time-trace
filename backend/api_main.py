...
def setup_middleware():
    ...
    app.add_middleware(
        CORSMiddleware,
        allow_origins=config_manager.get("api.cors_origins", ["http://localhost:3000", "http://127.0.0.1:3000"]),
    ...
    # 受信任主机中间件
    trusted_hosts = config_manager.get("api.trusted_hosts", ["localhost", "127.0.0.1"])
    app.add_middleware(
...
async def health_check():
    ...
    try:
        ...
        # 检查事件服务
        from event_service import event_service_manager
        service_status = event_service_manager.get_status()
        ...
        return create_api_response(
            ...
            data={
                ...
                "uptime": service_status.get("uptime_minutes", 0)
            }
        )
...
async def get_system_config():
    ...
    try:
        ...
        safe_config = {
            "work": {
                "standard_hours": config_manager.get("work.standard_hours", 8.0),
                "start_time": config_manager.get("work.start_time", "09:00"),
                "end_time": config_manager.get("work.end_time", "18:00"),
                "lunch_start": config_manager.get("work.lunch_start", "12:00"),
                "lunch_end": config_manager.get("work.lunch_end", "13:00")
            },
            "event": {
                "enabled_types": config_manager.get("event.enabled_types", []),
                "min_intervals": config_manager.get("event.min_intervals", {})
            },
            "api": {
                "cors_origins": config_manager.get("api.cors_origins", []),
                "trusted_hosts": config_manager.get("api.trusted_hosts", [])
            }
        }
...
async def startup_event():
    ...
    # 启动事件服务
    try:
        from event_service import start_event_service
        start_event_service()
...
async def shutdown_event():
    ...
    # 停止事件服务
    try:
        from event_service import stop_event_service
        stop_event_service()
        logger.info("事件服务已停止")
    except Exception as e:
